...
edit_dist_t sift4(std::string_view s1,
  ...
  auto lcss = 0;  // largest common subsequence
  auto local_cs = 0;  // local common substring
  auto trans = 0;  // number of transpositions ('ab' vs 'ba')
  while ((c1 < l1) && (c2 < l2)) {
    if (s1[c1] == s2[c2]) {
      local_cs++;
      auto isTrans = false;
      ...
      for (auto i = 0; i < maxOffset && (c1 + i < l1 || c2 + i < l2); i++) {
  ...
  return std::max(l1, l2) - lcss + trans;
}
